...
describe('Layout Weather app test', () => {
  ...
  describe('Markup test', () => {
    ...
    test('Header container with title icon and text exists', () => {
      ...
      expect(headerTitle.querySelector('i')).toHaveClass('fas', 'fa-cloud-sun');
    ...
    test('Footer container with copyright and sources elements and text exists', () => {
      ...
      expect(paragraphs[0].innerText).toContain('Weather Forecast StanSky App');

      expect(paragraphs[1]).not.toBeNull();
      expect(paragraphs[1].innerText).toContain(
    ...
    test('Map container with empty state exists', () => {
      ...
      const mapStateContainer = mapContainer.querySelector('.state-container');
      expect(mapContainer).not.toBeNull();
      expect(mapStateContainer).not.toBeNull();
      expect(mapStateContainer.querySelector('i')).toHaveClass('fas', 'fa-map');
    ...
    test('Sidebar with header exists', () => {
      ...
      const historySectionHeader = historyCard.querySelector('.section-header');
      ...
      expect(historySectionHeader.querySelector('h2 span').innerText).toContain(
        ' Search History ',
      );
    });
